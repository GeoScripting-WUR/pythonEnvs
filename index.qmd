...
<img src="images/positron/empty_positron.png" alt="Positron when you open it for the first time" width="100%"></img>
...
In your Positron terminal, go to the Documents directory and create a new pixi project `MyPackage`, by running:

```{Python, engine.path='/usr/bin/python3', eval=TRUE}
pixi init MyPackage
```

This line creates a new folder called MyPackage. Open it via Positron (or use 'cd MyPackage' in bash). When you are in the directory Documents/MyPacakge run
...
When you do this, a terminal pops up, showing the output of the file. In Positron we normally run code per file. In python, running code per file is the the normal way to go. However, you can also run the single lines as we are used to in R by using CTRL+ENTER, and see the output line by line in the Console window.
...
<img src="images/positron/main.py.png" alt="The main.py script" width="100%"></img>
...
To show you some of this functionality, write the following code in the `MyPackage.__init__.py` (create a new python file called "__init__.py" in the MyPackage folder and write on it): 
...
#(MyPackage) osboxes@geoscripting: ~Documents/MyPackage$
# now you can run
python main.py
...
#### REPL
Additionally to the debugger, you can play around with code in jupyter style notebooks. You can start the repl by opening the command palette (`Ctrl` + `Shift` + `P`) and type _repl_, and click _'Python: Start Native Python REPL'_. A new window will show up. You can create a new cell by hovering the mouse on the window and select 'Code'. If you type some python in the cell, for example `print('hello everybody!!')`. The code you ran shows up in a cell and the output in the cell below. This is how notebooks work, they contain code per cell and show the results in line. 
