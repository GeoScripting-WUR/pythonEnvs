...
Go back to the Documents directory and create a new pixi project `MyPackage`, by running:
...
When you do this, a terminal pops up, showing the output of the file. In Positron we run code per file. In python, running code per file is the the normal way to go. However, you can also run the single lines as we are used to in R by using CTRL+ENTER, and see the output line by line in the Console window.
...
First we need to set up our environment. We now create a new environment (called geotest) using pixi. In the terminal, type:
...
```{r, eval=FALSE,engine='bash'}
pixi add python numpy matplotlib geopandas spyder
```
